...
def get_weather (message):
    ...
    if weather.status_code == 200:
        ...
        # Берем температуру и округляем до целого
        temp = data['main']['temp']
        rounded_temp = round(temp)  # ← вот округление!
    
        bot.reply_to(message, f'На данный момент температура по °C в этом городе: {rounded_temp}')
